...
import { fetchGroups, fetchGroupStreaks, createStreakLog, deleteStreakLog, type StreakGroup, type StreakRecord } from '../api'
...
const calculateRunningStreaks = (allStreaks: { name: string; records: StreakRecord[] }[], dateRows: { date: string }[]): Map<string, number> => {
  const runningStreaks = new Map<string, number>()
  const allDatesInRange = dateRows.map(row => row.date)

  const countConsecutiveDaysFromIndex = (recordsMap: Map<string, boolean>, startIndex: number, increment: number): number => {
    let count = 0
    for (let i = startIndex; i >= 0 && i < allDatesInRange.length; i += increment) {
  ...
  allStreaks.forEach(streak => {
    ...
    const hasYesterdayRecord = yesterdayIndex >= 0 && recordsMap.get(allDatesInRange[yesterdayIndex])
    ...
    if (hasTodayRecord) {
      ...
      runningStreak = countConsecutiveDaysFromIndex(recordsMap, yesterdayIndex, -1)
    } else {
...
export default function StreakGroupTable() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const streakGroupPromises = groupsList.map(group => fetchGroupStreaks(group.id))
        const streakGroups = await Promise.all(streakGroupPromises)
        const validStreakGroups = streakGroups.filter((group): group is StreakGroup => group !== null)
  ...
  const dateRows = useMemo(() => {
    ...
    return allDates.map(date => ({
      ...
      records: new Map(allStreaks.map(streak => [
        streak.name,
  ...
  const streakLookup = useMemo(() => {
    const lookup = new Map<string, { groupIndex: number; streakIndex: number; streakId: number }>()
    streakData.forEach((group, groupIndex) => {
      group.streaks.forEach((streak, streakIndex) => {
        lookup.set(streak.name, { groupIndex, streakIndex, streakId: streak.id })
      })
    })
    return lookup
  }, [streakData])
  ...
  const toggleStreakRecord = async (streakName: string, date: string) => {
    ...
    try {
      ...
      setStreakData(prevData => {
        ...
        if (newPresent) {
          if (recordIndex >= 0) {
            updatedRecords[recordIndex] = { ...updatedRecords[recordIndex], present: true }
          } else {
  ...
  useEffect(() => {
    if (dateRows.length > 0 && virtuosoRef.current && !hasScrolledToBottom.current && !loading) {
      setTimeout(() => {
  ...
  if (loading) return <div className="virtuoso-table-container loading-container">Loading streak data...</div>
  ...
  if (allStreaks.length === 0) return <div className="virtuoso-table-container no-data-container">No streak data available</div>
      ...
      <TableVirtuoso
        ...
        fixedFooterContent={() => (
          <tr className="table-footer">
            <td colSpan={2} className="footer-label">Streak</td>
            {allStreaks.map(streak => (
        ...
        itemContent={(_index, dateRow) => {
          ...
          const allStreaksAbsent = allStreaks.every(streak => !dateRow.records.get(streak.name))
              ...
              {allStreaks.map(streak => {
                ...
                return (
                  ...
                  <td
                    ...
                    onClick={() => toggleStreakRecord(streak.name, dateRow.date)}
